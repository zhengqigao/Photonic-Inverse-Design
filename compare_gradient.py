...
def train(
    ...
    for batch_idx, (ori_batch, batch) in enumerate(zip(ori_train_loader, train_loader)):
        ori_wavelength, ori_grid_step, ori_data, ori_target = ori_batch
        wavelength, grid_step, data, target = batch
        #     break
        # for batch_idx, _ in enumerate(train_loader):
        ori_wavelength = ori_wavelength.to(device, non_blocking=True)
        ori_grid_step = ori_grid_step.to(device, non_blocking=True)
        ...
        ori_wavelength, ori_grid_step, ori_data, ori_target = [
            x.flatten(0, 1) for x in [ori_wavelength, ori_grid_step, ori_data, ori_target]
        ]
        ...
        with amp.autocast(enabled=grad_scaler._enabled):
            output = model(ori_data, wavelength, grid_step)
...
def main() -> None:
    ...
    curl_loss = CurlLoss(
        configs.model.grid_step, wavelength=train_loader.dataset[0][0][0].item()
    ).to(device)
