...
class MultiStepDynamicCNN(PDE_NN_BASE):
    def __init__(
        ...
        **kwargs,
        ...
        if self.if_pass_history:
            assert self.num_iters > 1, "if pass hidden state, num_iters must be larger than 1"
    ...
    def build_layers(self):
        ...
        if self.if_pass_history and (not self.share_history_encoder):
            history_encoder = [
                self.build_modules("history_encoder", self.history_encoder_cfg, self.backbone_cfg.kernel_list[-1]+self.encoder_cfg.kernel_list[-1])
        ...
        if not self.share_encoder:
            ...
            assert self.num_iters == len(object_list), "the encoder must be different with each other"
        else:
            encoder = [
                self.build_modules("encoder", self.encoder_cfg, self.in_channels)
            ] * self.num_iters
        ...
        if not self.share_backbone:
            ...
            assert self.num_iters == len(object_list), "the backbone must be different with each other"
        else:
            backbone = [
                self.build_modules("backbone", self.backbone_cfg, new_out_channels)
            ] * self.num_iters
        ...
        if not self.share_decoder:
            ...
            assert self.num_iters == len(object_list), "the decoder must be different with each other"
        else:
            decoder = [
                self.build_modules("decoder", self.decoder_cfg, new_out_channels)[0]
            ] * self.num_iters
        ...
        if not self.share_history_encoder and src > 1:
            if hasattr(optimizer, "state"):
                for p_src, p_dst in zip(
                    ...
                    optimizer.state[p_dst] = copy.deepcopy(optimizer.state[p_src])
            self.history_encoder[dst-1].load_state_dict(self.history_encoder[src-1].state_dict())
    ...
    def forward(
        ...
        for iter, src in enumerate(srcs):
            ...
            input_fields = out[:, -self.in_frames :, ...] if self.if_pass_grad else out[:, -self.in_frames :, ...].detach()
            if self.field_norm_mode != "none":
