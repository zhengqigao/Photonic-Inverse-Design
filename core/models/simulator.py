...
def maxwell_residual_2d_complex(
    fields_re,
    fields_im,
    epsilons,
    curl_curl_re,
    ...
    # Compute Maxwell operator on fields
    curl_curl_E_re = (SCALE / L0 ** 2) * (
        torch.matmul(curl_curl_re, fields_re) - torch.matmul(curl_curl_im, fields_im)
    )
    curl_curl_E_im = (SCALE / L0 ** 2) * (
        torch.matmul(curl_curl_im, fields_re) + torch.matmul(curl_curl_re, fields_im)
    )

    epsilon_E_re = (SCALE * -(OMEGA_1550 ** 2) * MU0 * EPSILON0) * epsilons * fields_re
    epsilon_E_im = (SCALE * -(OMEGA_1550 ** 2) * MU0 * EPSILON0) * epsilons * fields_im

    out_re = curl_curl_E_re - epsilon_E_re
    out_im = curl_curl_E_im - epsilon_E_im

    out_re = out_re.view(batch_size, W, H)
    out_im = out_im.view(batch_size, W, H)

    if trim_buffer and buffer_length > 0:
        clip = buffer_length + npml
        return out_re[:, clip:-clip, clip:-clip], out_im[:, clip:-clip, clip:-clip]
    else:
...
class Simulation2D:
    ...
    def solve2(self, epsilons: np.array, omega=OMEGA_1550, src_x=None, src_y=None, clip_buffers=False):
        ...
        sim.src[src_y[0] + self.npml + self.buffer_length:src_y[1] + self.npml + self.buffer_length, src_x[0] + self.npml + self.buffer_length:src_x[1] + self.npml + self.buffer_length] = 1j
