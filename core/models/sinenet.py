...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, padding_mode, num_groups=1, norm: bool = True, activation="gelu") -> None:
        ...
        if norm:
            ...
            # self.norm1 = MyLayerNorm(out_channels, data_format="channels_first") # use our own layer norm instead of GroupNorm
...
class sinenet(nn.Module):
    ...
    def __init__(
        ...
        insize = time_history * (self.n_input_scalar_components + self.n_input_vector_components * 2) + 1 + self.time_future# this one represent for the eps
    ...
    def forward(
        ...
        x = torch.where(src_mask, src[:, -self.time_future:, ...]/scaling_factor, x)
        x = x*padding_mask
