...
default_UniRepLKNet_1_kernel_sizes = (
    (3, 3),
    (13, 13),
    (13, 13, 13, 13),
    (13, 13),
...
default_depths_to_kernel_sizes = {
    UniRepLKNet_1_depths: default_UniRepLKNet_1_kernel_sizes,
...
class UniRepLKNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        in_chans = in_chans + 0  # pos_encoding
        ...
        self.downsample_layers.append(
            nn.Sequential(
                ...
                nn.Conv2d(in_chans, dims[0] // 2, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x, grid_step: Tensor, wavelength: float = 1.55):
        ...
        src = x[:, -self.out_chans :]
        # input = x[:, -self.out_chans:]
        input_size = x.shape
        ## must normalize field, because the light propagation principle is not related to magnitude
        # input_field = x[:, 1 : 1 + self.out_chans]
        # max_field = input_field.detach().abs().max() + 1e-6
        # x = torch.cat(
        #     [x[:, 0:1], input_field, src], dim=1
        # )
...
def unireplknet_1(**kwargs):
    model = UniRepLKNet(
        depths=[2, 2, 4, 2], dims=(16, 32, 64, 128), **kwargs
    )
    return model
