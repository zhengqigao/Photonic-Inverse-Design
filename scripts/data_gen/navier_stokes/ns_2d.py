...
Date: 2021-12-26 14:34:07
LastEditors: Jiaqi Gu (jqgu@utexas.edu)
LastEditTime: 2021-12-26 14:34:07
"""
...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    ...
    w_h = torch.view_as_real(torch.fft.fft2(w0.to(torch.cfloat), norm="forward"))

    # Forcing to Fourier space
    # f_h = torch.rfft(f, 2, normalized=False, onesided=False)
    f_h = torch.view_as_real(torch.fft.fft2(f.to(torch.cfloat), norm="forward"))
    ...
    for j in range(steps):
        ...
        psi_h = w_h.clone()
        # print(psi_h.size(), lap.size())
        psi_h[..., 0] = psi_h[..., 0] / lap.unsqueeze(0)
        psi_h[..., 1] = psi_h[..., 1] / lap.unsqueeze(0)
        ...
        q = torch.fft.irfft2(torch.view_as_complex(q), (N, N), norm="forward")
        ...
        v = torch.fft.irfft2(torch.view_as_complex(v), (N, N), norm="forward")
        ...
        w_x = torch.fft.irfft2(torch.view_as_complex(w_x), (N, N), norm="forward")
        ...
        w_y = torch.fft.irfft2(torch.view_as_complex(w_y), (N, N), norm="forward")

        # Non-linear term (u.grad(w)): compute in physical space then back to Fourier space
        # F_h = torch.rfft(q * w_x + v * w_y, 2, normalized=False, onesided=False)
        F_h = torch.view_as_real(torch.fft.fft2((q * w_x + v * w_y).to(torch.cfloat), norm="forward"))
        ...
        w_h[..., 0] = (
            -delta_t * F_h[..., 0] + delta_t * f_h[..., 0] + (1.0 - 0.5 * delta_t * visc * lap) * w_h[..., 0]
        ) / (1.0 + 0.5 * delta_t * visc * lap)
        w_h[..., 1] = (
            -delta_t * F_h[..., 1] + delta_t * f_h[..., 1] + (1.0 - 0.5 * delta_t * visc * lap) * w_h[..., 1]
        ) / (1.0 + 0.5 * delta_t * visc * lap)
        ...
        if (j + 1) % record_time == 0:
            ...
            w = torch.fft.irfft2(torch.view_as_complex(w_h), (N, N), norm="forward")
...
ensure_dir("data/navier_stokes/")
scipy.io.savemat("data/navier_stokes/ns_data.mat", mdict={"a": a.cpu().numpy(), "u": u.cpu().numpy(), "t": sol_t.cpu().numpy()})
