...
Date: 2022-04-20 13:40:22
LastEditors: Jiaqi Gu (jqgu@utexas.edu)
LastEditTime: 2022-04-28 02:53:27
'''
import os
import numpy as np
import subprocess
...
root = f"log/{dataset}/{model}/{exp_name}_dynamics"
script = "test.py"
...
def task_launcher(args):
    ...
    with open(os.path.join(root, f"{dataset}_{test_mode}_mixup-{mixup}_id-{id}.log"), "w") as wfid:
        exp = [
            ...
            f"--plot.interval=50",
            f"--plot.dir_name=dynamics_spec_mix_{model}_{exp_name}_{dataset}_mixup-{mixup}_cmae_exp_nl-{n_layer}_{test_mode}_{id}",
