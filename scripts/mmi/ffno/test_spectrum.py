...
root = f"log/{dataset}/{model}/{exp_name}_spectrum"
script = "test.py"
...
def task_launcher(args):
    ...
    dataset, dataset_dir, test_mode, mixup, ckpt, id = args
    n_layer = 12

    # data_list = [f"{dataset}_{int(i*4)}" for i in range(n_data)]
    with open(os.path.join(root, f"{dataset}_{test_mode}_mixup-{mixup}_id-{id}.log"), "w") as wfid:
        exp = [
            f"--dataset.pol_list={[dataset]}",
...
if __name__ == "__main__":
    ...
    tasks = [
        (
            "cband_rHz_mmi4x4_3pads",
            "cband_rHz_mmi4x4_3pads_4",
            "mm",
            1,
            "./checkpoint/mmi/ffno/train_random/FFNO2d__err-0.1207_epoch-195.pt",
            1,
        ),
        # ("rHz", "random_size5", "mm", 1, "./checkpoint/mmi/ffno/train_random/FFNO2d__err-0.1207_epoch-195.pt", 1),
    ]
    tasks = [
        (
            f"cband_rHz_mmi3x3_3pads_{i}",
            f"cband_rHz_mmi3x3_3pads_{i}",
            "mm",
            1,
            "./checkpoint/mmi/ffno/train_random/FFNO2d__err-0.1207_epoch-195.pt",
