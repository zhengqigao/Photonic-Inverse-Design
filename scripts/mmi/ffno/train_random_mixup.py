...
LastEditTime: 2022-03-18 00:23:36
'''
import os
import numpy as np
import subprocess
...
root = f"log/{dataset}/{model}/{exp_name}_mixup"
script = 'train.py'
...
def task_launcher(args):
    ...
    with open(os.path.join(root, f'rHz10_data-{ratio:.2f}_id-{id}.log'), 'w') as wfid:
        exp = [
            ...
            f"--plot.interval=50",
            f"--plot.dir_name={model}_{exp_name}_rHz10_mixup-0_cmae_exp_nl-{n_layer}_data-{ratio:.2f}_{id}",
            ...
            f"--checkpoint.model_comment=data-{ratio:.2f}_mixup-0"
            ]
