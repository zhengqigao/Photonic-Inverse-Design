...
LastEditTime: 2022-03-03 23:53:45
'''
import os
import subprocess
...
def task_launcher(args):
    ...
    with open(os.path.join(root, f'rHz{n_data}_mixup-{mixup}_loss-{loss}_enc-{enc}_nl-{n_layer}_id-{id}.log'), 'w') as wfid:
        exp = [
            f"--dataset.pol_list={str(data_list)}",
            f"--plot.interval=10",
            f"--plot.dir_name={model}_{exp_name}_rHz{n_data}_mixup-{mixup}_{loss}_{enc}_nl-{n_layer}_{id}",
...
if __name__ == '__main__':
    ...
    tasks = [[1, "cmae", 0.002, "exp", 5, 8, 1]] # deep, bsconv stem, augpath
    tasks = [[1, "cmae", 0.002, "exp", 5, 8, 2]] # deep, prenorm bsconv stem, augpath
    # tasks = [[0, 1]]
