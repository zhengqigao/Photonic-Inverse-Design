...
def train(
    ...
    model.train()
    configs.run.multi_train_schedule = configs.run.multi_train_schedule[:model.num_iters-1]
    ...
    for batch_idx, (raw_data, raw_target) in enumerate(train_loader):
        ...
        target = target[:, :model.out_channels*(configs.run.multi_train_milestone+1)]
        ...
        with amp.autocast(enabled=grad_scaler._enabled):
            ...
            if maskloss:
                if model.num_iters == 1:
                    ...
                    regression_loss = criterion(output, target/normalization_factor, raw_data["mseWeight"], model.num_iters)
            else:
                regression_loss = criterion(output, target/normalization_factor, raw_data["mseWeight"])
