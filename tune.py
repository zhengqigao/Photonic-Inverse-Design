...
def train(
    ...
    for batch_idx, (wavelength, grid_step, data, target) in enumerate(train_loader):
        ...
        # if curl_loss_weight > 0:
        #     curl_error = criterion(
        #         curl_loss(output[:, :, 10:-10, 100:-100], data[:, 0:1, 10:-10, 100:-100], reduction="none"),
        #         curl_loss(target[:, :, 10:-10, 100:-100], data[:, 0:1, 10:-10, 100:-100], reduction="none"),
        #     )
        #     loss = loss + curl_loss_weight * curl_error
        # else:
        #     curl_error = torch.zeros_like(loss.data)
        # f_loss = criterion(torch.fft.fft2(output, norm="ortho"),torch.fft.fft2(target, norm="ortho"))
        # loss = loss + f_loss
        # loss_e = poynting_loss(output, target)
        # loss = loss + 10 * loss_e

        # curl_meter.update(curl_error.item())
...
def validate(
    ...
    with torch.no_grad():
        for i, (wavelength, grid_step, data, target) in enumerate(validation_loader):
            ...
            wavelength, grid_step, data, target = [
                x.flatten(0, 1) for x in [wavelength, grid_step, data, target]
            ]
            output = model(data, wavelength, grid_step)
...
def main() -> None:
    ...
    curl_loss = CurlLoss(configs.model.grid_step, wavelength=train_loader.dataset[0][0][0].item()).to(device)
