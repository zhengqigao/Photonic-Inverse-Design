...
def plot_gaussian():
    ...
    runs = []
    for std in stds:
        log = f"darcy_istd-{std}_tstd-{std}_is-1_ts-1_ip-fp32_tp-fp32_id-1.log"
        with open(f"./log/darcy/fno/train_noise_grad/{log}", "r") as f:
            lines = f.readlines()
    ...
    for run_id in runs:
        for metric in metrics:
            x, y = parse_mlflow_trace(exp_id=614905274718630797, run_id=run_id, metric=metric)
            x = x[:10]
            y = y[:10]
...
def plot_quantization():
    ...
    runs = []
    for prec in precs:
        log = f"darcy_istd-0_tstd-0_is-1_ts-1_ip-{prec}_tp-{prec}_id-1.log"
        with open(f"./log/darcy/fno/train_noise_grad/{log}", "r") as f:
            lines = f.readlines()
            p = re.compile(r".*Run ID: \((\S+)\).*")
            for line in lines:
    ...
    fig, ax = None, None
    name = "TrainGradQuantization_Darcy"
    scale = 1e11
...
def plot_downsample():
    ...
    for factor in factors:
        log = f"darcy_istd-0_tstd-0_is-{factor}_ts-{factor}_ip-fp32_tp-fp32_id-1.log"
        with open(f"./log/darcy/fno/train_noise_grad/{log}", "r") as f:
            lines = f.readlines()
            p = re.compile(r".*Run ID: \((\S+)\).*")
            for line in lines:
    ...
    fig, ax = None, None
    name = "TrainGradDownsample_Darcy"
    scale = 1e11

    markers = ["o", "X", "^", "v", "*", "<", "s"]
 
    for i, std in enumerate([0.8, 0.7, 0.6, 0.5]):
        fig, ax, _ = batch_plot(
            "line",
            raw_data={
                "x": x,
